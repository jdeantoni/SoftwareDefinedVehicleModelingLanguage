...
function prettyPrintPeriodiComponent(c: Component, ifContent: CompositeGeneratorNode) {
    ...
    state processing;
        ...
        when x >= ${s.execTime.mean - 2 * s.execTime.stdDev} and x <= ${s.execTime.mean + 2 * s.execTime.stdDev};
            ...
            output ${c.publishers.at(0)?.name}() to {TODO}0;
            reset x;
            nextstate wait;
    endstate;\n`);
...
function prettyPrintActuatorSignal(sig: Actuator, ifContent: CompositeGeneratorNode, sigName: string) {
